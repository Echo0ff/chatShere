...
class DeployManager:
    ...
    def deploy_production(self):
        ...
        # 运行数据库迁移
        self.run_command(
            "docker-compose -f docker-compose.prod.yml exec chatsphere python scripts/setup.py"
        )
...
def main():
    ...
    parser.add_argument(
        "environment", choices=["development", "testing", "production"], help="部署环境"
    )
    parser.add_argument(
        "--action", choices=["deploy", "stop", "backup"], default="deploy", help="执行的动作"
    )
