...
class CacheManager:
    ...
    async def hash_set(self, key: str, field: str, value: Any) -> bool:
        ...
        try:
            cache_key = self._make_key(key)
            serialized_value = json.dumps(value, ensure_ascii=False) if isinstance(value, (dict, list)) else str(value)
...
class ChatCacheManager(CacheManager):
    ...
    # 速率限制
    async def check_rate_limit(self, user_id: str, action: str, limit: int = 10, window: int = 60) -> bool:
        """检查速率限制"""
